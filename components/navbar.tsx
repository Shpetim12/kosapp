...
export function Navbar({ user, role, isAdmin }: NavbarProps) {
  ...
  return (
    <header className={`sticky top-0 z-40 bg-white/80 backdrop-blur-md transition-all ${scrolled ? "border-b border-slate-200 shadow-sm" : "border-b border-transparent"}`}>
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex items-center gap-2 text-lg font-black tracking-tight text-slate-950">
          <span className="grid h-10 w-10 place-items-center rounded-2xl bg-brand-600 text-white shadow-lg shadow-green-600/20">
            <Home size={19} />
          </span>
          KosBanesa
        </Link>
      ...
      {open ? (
        <div className="fixed inset-0 z-50 bg-slate-950/40 lg:hidden" onClick={() => setOpen(false)}>
          <aside
            className="absolute right-0 top-0 flex h-dvh w-[86vw] max-w-sm flex-col bg-white p-5 shadow-2xl"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="mb-8 flex items-center justify-between">
              <Link href="/" className="flex items-center gap-2 text-lg font-black text-slate-950" onClick={() => setOpen(false)}>
                <span className="grid h-10 w-10 place-items-center rounded-2xl bg-brand-600 text-white">
                  <Home size={19} />
                </span>
                KosBanesa
              </Link>
              <button className="grid h-11 w-11 place-items-center rounded-2xl border border-slate-200" onClick={() => setOpen(false)} aria-label="Mbyll menunë">
                <X size={20} />
              </button>
            </div>

            <nav className="grid gap-2">
              <Link href="/listings" onClick={() => setOpen(false)} className="flex min-h-12 items-center gap-3 rounded-2xl px-4 font-semibold text-slate-800 hover:bg-slate-50">
                <Search size={18} /> Kërko
              </Link>
              <Link href="/#si-funksionon" onClick={() => setOpen(false)} className="flex min-h-12 items-center rounded-2xl px-4 font-semibold text-slate-800 hover:bg-slate-50">
                Si funksionon
              </Link>
              {role === "landlord" ? (
                <Link href="/dashboard" onClick={() => setOpen(false)} className="flex min-h-12 items-center gap-3 rounded-2xl px-4 font-semibold text-slate-800 hover:bg-slate-50">
                  <LayoutDashboard size={18} /> Paneli im
                </Link>
              ) : null}
              {isAdmin ? (
                <Link href="/admin" onClick={() => setOpen(false)} className="flex min-h-12 items-center gap-3 rounded-2xl px-4 font-semibold text-slate-800 hover:bg-slate-50">
            ...
            <div className="mt-auto border-t border-slate-100 pt-5">
              {user ? (
                <div className="flex items-center gap-3 rounded-2xl bg-slate-50 p-3">
                  <div className="grid h-11 w-11 place-items-center rounded-full bg-brand-600 font-bold text-white">
