    ...
    <main className="grid min-h-[calc(100vh-64px)] place-items-center bg-[#0f0f1a] px-4 py-10">
      <form onSubmit={onSubmit} className={`focus-card w-full max-w-md space-y-6 p-6 sm:p-8 ${message ? "animate-shake" : ""}`}>
        <div className="text-center">
          ...
          <h1 className="mt-5 text-3xl font-black text-white">Mirë se u kthyet</h1>
          <p className="mt-2 text-sm text-[#a0a0b0]">Hyni për të vazhduar në Strehë.</p>
        </div>

        <label className="block space-y-2">
          <span className="label">Email</span>
          <input
            className={`field ${message ? "border-red-500 focus:border-red-500 focus:ring-red-500/20" : ""}`}
            type="email"
            value={email}
        ...
        <label className="block space-y-2">
          ...
          <div className="relative">
            <input
              className={`field pr-12 ${message ? "border-red-500 focus:border-red-500 focus:ring-red-500/20" : ""}`}
            ...
            <button
              type="button"
              className="absolute right-2 top-1/2 grid h-10 w-10 -translate-y-1/2 place-items-center rounded-xl text-[#a0a0b0] hover:bg-white/5"
        ...
        <div className="space-y-3 text-center text-sm">
          <p className="text-[#a0a0b0]">
            Nuk ke llogari?{" "}
            <Link href="/signup" className="font-black text-[#e8b04b]">
              Regjistrohu
            </Link>
          </p>
          <Link href="/listings" className="inline-flex font-bold text-[#a0a0b0] hover:text-white transition-colors">
            Vazhdo si vizitor
          </Link>
        </div>
      </form>
